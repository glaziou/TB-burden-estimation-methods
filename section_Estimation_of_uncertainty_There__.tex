...
where $E(y)$ denotes the expectation of $y$, $\sigma^2 y$ is the variance of $y$, $\nabla_x$ is the $p \times n$ gradient matrix with all partial first derivatives, $C_x$ = the $p \times p$ covariance matrix. 

The first order Taylor expansion assumes linearity over $\bar {x_i}$. We used second-order expansions to correct for bias in non-linear expressions. This approach was implemented whenever possible to shorten computing time.
